...
Route::get('/cat-profile/{cat}', function (Cat $cat) {
    ...
    return Inertia::render('CatProfile', [
        'cat' => [
            ...
            'size' => $cat->size,
            'color' => $cat->color,
            'weight' => $cat->weight_kg ? "{$cat->weight_kg} kg" : null,
            'location' => $cat->location,
            'story' => $cat->rescue_story,
            'image' => $cat->images->first()->path ?? $cat->photo_path ?: '/images/gallery-cat.png',
            'images' => $cat->images->pluck('path')->values()->all(),
            'specialMedicalNeeds' => collect($cat->special_medical_needs ?: [])->values()->all(),
            'tags' => collect($cat->profile_tags ?: [])->values()->all(),
            'personality' => collect($cat->personality_traits ?: [])->values()->all(),
            'fivStatus' => $cat->fiv_status,
            'felvStatus' => $cat->felv_status,
            'fipHistory' => $cat->fip_history,
            'spayNeuterStatus' => $cat->spay_neuter_status,
            'microchipStatus' => $cat->microchip_status,
            'vaccinationStatus' => $cat->vaccination_status,
            'currentMedication' => $cat->current_medication,
            'energyLevel' => $cat->energy_level,
            'socialBehavior' => $cat->social_behavior,
            'idealHomeType' => $cat->ideal_home_type,
            'handlingTolerance' => $cat->handling_tolerance,
            'dailyAttentionRequirement' => $cat->daily_attention_requirement,
            'goodWithCats' => $cat->good_with_cats,
            'goodWithDogs' => $cat->good_with_dogs,
            'goodWithChildren' => $cat->good_with_children,
            'dietType' => $cat->diet_type,
            'groomingNeeds' => $cat->grooming_needs,
            'categories' => $cat->categories->map(fn ($category) => [
                'id' => $category->id,
                'name' => $category->name,
...
Route::middleware(['auth', 'verified', 'role:admin'])->group(function () {
    ...
    Route::post('/admin/cat-options', [CatController::class, 'storeOption'])->name('admin.cat-options.store');
    Route::put('/admin/cat-options', [CatController::class, 'updateOption'])->name('admin.cat-options.update');
