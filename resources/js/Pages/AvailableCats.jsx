...
const compatibilityOptions = [
    ...
    { value: 'good_with_dogs', label: 'Friendly to other dogs' },
    { value: 'apartment_friendly', label: 'Good for Apartments' },
    { value: 'good_with_children', label: 'Friendly with Kids' },
];
...
function FilterGroup({ title, items, selected = [], onToggle }) {
    ...
    return (
        <div className="bg-[#F6EDE5] p-5 rounded-2xl">
            <h4 className="text-xs font-bold text-[#f08063] uppercase tracking-wider mb-4">{title}</h4>
...
export default function AvailableCats({ availableCats = [], filterOptions = {}, selectedFilters = {} }) {
    const activeFilters = {
        ...
        personality: selectedFilters.personality || [],
        compatibility: selectedFilters.compatibility || [],
        vaccination: selectedFilters.vaccination || [],
    };
    ...
    const toggleFilter = (group, value) => {
        ...
        const nextGroupValues = current.includes(value)
    ...
    const clearFilters = () => {
        router.get(route('cats.available'), {}, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
                    ...
                    <aside className="w-full lg:w-72 flex-shrink-0">
                        <div className="bg-white rounded-[32px] p-4 shadow-sm border border-gray-100 sticky top-24">
                            <div className="flex items-center justify-between gap-3 mb-8">
                                ...
                                {hasActiveFilters ? (
                                    <button
                                        ...
                                        className="text-xs font-semibold text-[#f08063] hover:underline"
                                    >
                                        Clear all
                                    </button>
                                ) : null}
                            ...
                            <div className="space-y-8">
                                <FilterGroup title="Breed" items={filterOptions.breed || []} selected={activeFilters.breed} onToggle={(v) => toggleFilter('breed', v)} />
                                <FilterGroup title="Age" items={filterOptions.age || []} selected={activeFilters.age} onToggle={(v) => toggleFilter('age', v)} />
                                <FilterGroup title="Gender" items={filterOptions.gender || []} selected={activeFilters.gender} onToggle={(v) => toggleFilter('gender', v)} />
                                <FilterGroup title="Personality" items={filterOptions.personality || []} selected={activeFilters.personality} onToggle={(v) => toggleFilter('personality', v)} />
                                <FilterGroup title="Compatibility" items={compatibilityOptions} selected={activeFilters.compatibility} onToggle={(v) => toggleFilter('compatibility', v)} />
                                <FilterGroup title="Vaccination" items={filterOptions.vaccination || []} selected={activeFilters.vaccination} onToggle={(v) => toggleFilter('vaccination', v)} />
                            </div>
                        </div>
                    </aside>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            ...
                            {availableCats.length === 0 && (
                                <div className="md:col-span-2 xl:col-span-3 rounded-2xl border border-gray-100 bg-white p-8 text-center text-gray-500">
                                    No cats found for selected filters.
                                </div>
                            )}
                        </div>
                    </div>
